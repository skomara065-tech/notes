...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const chkTunnel = document.getElementById('chkTunnel');
  const chkCapture = document.getElementById('chkCapture');
  const chkDispatch = document.getElementById('chkDispatch');
  const chkGemini = document.getElementById('chkGemini');
  ...
  async function pollTokens() {
    ...
    try {
      ...
      // Check if we received text/html (localtunnel splash screen blocked) or real JSON data
      const contentType = res.headers.get('content-type');
      if (res.ok && contentType && contentType.includes('application/json')) {
        if(chkServer) chkServer.checked = true;
        if(chkTunnel) chkTunnel.checked = true;
        ...
        // It's hitting localtunnel, but blocked by the "Click to Continue" splash screen
        if(chkServer) chkServer.checked = false;
        if(chkTunnel) chkTunnel.checked = false;
      }
    } catch(e) {
      // Server is entirely off or URL is totally invalid
      if(chkServer) chkServer.checked = false;
      if(chkTunnel) chkTunnel.checked = false;
    }
    setTimeout(pollTokens, 10000);
  ...
  btnStart.addEventListener('click', () => {
    // Clear past runtime diagnostic memory flags on fresh click
    if(chkCapture) chkCapture.checked = false;
    if(chkDispatch) chkDispatch.checked = false;
    if(chkGemini) chkGemini.checked = false;
    ...
    chrome.runtime.sendMessage({ action: 'startRecording', config }, (res) => {
      ...
      if (res && res.status === 'started') {
        updateUI(true);
        if(chkCapture) chkCapture.checked = true; // Audio stream is successfully locked
      }
  ...
  port.onMessage.addListener((req) => {
    if (req.action === 'newNotePreview') {
      if(chkDispatch) chkDispatch.checked = true; // The file successfully departed offscreen.js
      
      if (req.summary.startsWith('⏳')) {
        showToast(req.summary, 'warning');
      } else if (req.summary.startsWith('❌')) {
        if(chkGemini) chkGemini.checked = false; // Something crashed on the server
        showToast(req.summary, 'error');
      } else {
        if(chkGemini) chkGemini.checked = true; // Handshake complete! Gemini responded successfully
